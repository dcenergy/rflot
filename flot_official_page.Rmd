...
#This needed in some version combinations of rmarkdown + data.table
...
### Using rflot
Note, rflot is designed to work exclusively with objects of type `data.table`.

#### Basic
Let us begin with a simple chart of gas versus temperature.  The function `flotChart` is used to specify the main data-source for the chart, as well as mannually adjust the height and width of the plot.  It needs to be paired with the `flotSeries` function, which adds a layer/series to the chart.
```{r}
flotChart(data.table(airquality)) %>%
...
flotChart(data.table(airquality)) %>%
  ...
  flotSeries(x=seq(min(Temp), max(Temp), .1)
             ,y=predict(lm(Ozone ~ Temp + I(Temp^2) + I(Temp^3)), newdata = data.table(Temp=seq(min(Temp), max(Temp), .1)))
             ,label="")
```
              ...
              ,tooltipOpts = list(             #Customize tool-tip options
                ...
                ,content = htmlwidgets::JS("
                  function(label, xval, yval, flotItem) {
                    return 'Day: ' + flotItem.series.extra_data[flotItem.dataIndex]['Day'] + 
                            '<br>Solar.R: ' + flotItem.series.extra_data[flotItem.dataIndex]['Solar.R'] +
                            '<br>Wind: ' + flotItem.series.extra_data[flotItem.dataIndex]['Wind'];
                  }
                ")
...
```{r}
flotChart(data.table(read.csv("msft_aapl_stock_data.csv"))) %>%
  ...
  flotOptions(xaxis=list(axisLabel = 'Date', mode='time')
              ,yaxes=list(
                list(axisLabel='MSFT')
                ,list(axisLabel='AAPL', position= "right", color='red',tickColor='red', font=list(color='red')))
